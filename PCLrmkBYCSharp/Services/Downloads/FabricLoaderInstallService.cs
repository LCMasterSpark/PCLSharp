...
public sealed class FabricLoaderInstallService(
    IDownloadByteClient client,
    IDownloadSourceService sources,
    IAppLoggerService logger) : IFabricLoaderInstallService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.Info("Fetching Fabric loader profile: " + url);
        ...
        var profile = JsonNode.Parse(bytes)?.AsObject()
            ?? throw new InvalidDataException("Invalid Fabric loader profile.");
        ...
        return new LoaderInstallPlan("fabric-loader", loaderVersion, versionJsonPath, files.DistinctBy(file => file.LocalPath, StringComparer.OrdinalIgnoreCase).ToList(), []);
    }
    ...
    private DownloadFile? CreateLibraryDownload(string minecraftRootPath, JsonObject library)
    {
        ...
        var repository = library["url"]?.GetValue<string>() ?? "https://maven.fabricmc.net/";
